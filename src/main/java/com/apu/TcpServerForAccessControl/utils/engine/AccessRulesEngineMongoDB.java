...
import com.apu.TcpServerForAccessControl.utils.database.MongoDBUtils;
...
@Component
public class AccessRulesEngineMongoDB {
...
//        mongoDBUtils.fillRepositoriesTestData();
...
//            }
        ...
        if(retPacket == null) {
            ruleList = ruleRepository.findByDeviceIdAndCardId(device.getDeviceId(), card.getCardId());
            if(ruleList.size() == 0) {
                AccessMessageWrong accessMessWrong = 
        ...
        if(retPacket == null) {
            for(Rule rule:ruleList) {
                int ruleEventId = rule.getEventId();
                ...
                if((dateBegin != null) && 
                   (dateEnd != null)) {
                    ...
                    AccessMessage accessMess = 
                            new AccessMessage(device.getDeviceId(), 
                                                card.getCardId(), 
                                                eventType.getEventId(), 
                                                dateTime, 
                                                "Access OK");
                    accessMessageRepository.insert(accessMess);
                    
                    //write to event_message
                    EventMessage eventMessage = 
                            new EventMessage(device.getDeviceId(), 
                                            eventTypeListTemp.get(0).getEventId(), 
                                            accessMess.getAccessMessId(), 
                                            rule.getRuleId(), 
                                            new Date(), 
                                            "Access allow");                  
                    eventMessageRepository.insert(eventMessage);
                }
            }
...
//        logger.info(msg);
        ...
        logger.info("Time inside accessRulesEngine: " + (timeReturnMessageReady - timeReceiveMesage)/1000 + " us" +
                        " = " 
                      + (timeBeforeGetDeviceFromDB - timeReceiveMesage)/1000 + " + " 
                      + (timeBeforeGetCardIdFromDB - timeBeforeGetDeviceFromDB)/1000 + " + " 
                      + (timeBeforeGetEventTypeFromDB - timeBeforeGetCardIdFromDB)/1000 + " + "
                      + (timeBeforeGetRuleFromDB - timeBeforeGetEventTypeFromDB)/1000 + " + "
                      + (timeBeforeCheckTimeBorders - timeBeforeGetRuleFromDB)/1000 + " + "
                      + (timeReturnMessageReady - timeBeforeCheckTimeBorders)/1000 + ";");
        return retPacket;
